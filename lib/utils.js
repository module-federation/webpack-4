 ...
 * @param {*} context 
 */
module.exports.getEntrysPath = function(dep, context) {
  if (dep.request) {
    // 绝对路径直接返回, 相对路径拼上context
    return /^(.:|\/)/.test(dep.request) ? [dep.request] : [path.join(context, dep.request)]
  }
  return dep.dependencies
    .filter(dep => dep.constructor.name.indexOf("EntryDependency") > -1)
    .map(dep => module.exports.getEntrysPath(dep, context)[0])
}
