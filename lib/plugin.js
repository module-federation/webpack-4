 ...
 */
 ...
 module.exports = class MFWebpack4 {
   ...
   convertRemotes(compiler) {
    ...
    //   compiler.options.resolve.alias[key] = `wpmjs/$/__mfplugin__${this.options.name}_usedShare_${key}`
    // })
    return remotes
   }
   ...
   interceptImport(resolver, compiler) {
     resolver.hooks.resolve.tapAsync(pluginName, (request, resolveContext, cb) => {
       ...
       Object.keys(this.options.shared).filter(pkgname => {
        ...
        if (new RegExp(`./${pkgname}(/|$)`).test(requestStr) && !new RegExp(`/node_modules/${pkgname}(/|$)`).test(issuer)) {
          pkgName = `mfshare:${pkgname}`
        }
      })
