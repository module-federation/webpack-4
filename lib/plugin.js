 ...
 module.exports = class ImportHttpWebpackPlugin {
   ...
   mergeAlias(compiler) {
     ...
     Object.keys(remotes).forEach(name => {
      let targetAlias = typeof remotes[name] === "string" ? remotes[name] : remotes[name].url
      if (!(/^https?:\/\//.test(targetAlias))) {
        targetAlias = `wpmjs/$/${name}`
      }
      if (compilerAlias[name]) {
        console.log(`[import-http-webpack-plugin]覆盖alias: ${compilerAlias[name]} -> ${targetAlias}`)
      }
      compilerAlias[name] = targetAlias
    })
   }
